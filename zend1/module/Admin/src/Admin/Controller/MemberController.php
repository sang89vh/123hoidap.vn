<?php
/**
 * Zend Framework (http://framework.zend.com/)
 *
 * @link      http://github.com/zendframework/Admin for the canonical source repository
 * @copyright Copyright (c) 2005-2012 Zend Technologies USA Inc. (http://www.zend.com)
 * @license   http://framework.zend.com/license/new-bsd New BSD License
 */
namespace Admin\Controller;

use FAQ\FAQCommon\FAQAbstractActionController;
use FAQ\Mapper\UserMapper;
use FAQ\FAQCommon\Util;
use Exception;
use Zend\Json\Json;
use Zend\View\Model\ViewModel;
use FAQ\FAQCommon\Usercfg;

class MemberController extends FAQAbstractActionController
{

    public function indexAction()
    {
        $privilege = Util::isPrivilege($this);
        if(!$privilege['isAllowed']){
        	return  $this->toNoticeError("Không được cấp quyền truy cập!",3000,"/") ;
        }
        $this->setLayoutAdmin();
        return array();
    }

    public function listUserAction()
    {
        $privilege = Util::isPrivilege($this);
        if(!$privilege['isAllowed']){
        	return  $this->toNoticeError("Không được cấp quyền truy cập!",3000,"/") ;
        }
        $this->setLayoutAjax();
        /*
         * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * Easy set variables
         */

        /* Array of database columns which should be read and sent back to DataTables. Use a space where
         * you want to insert a non-database field (for example a counter or static image)
        */
        $aColumns = array(
            "status",
            "id",
            "id",
            "first_name",
            "last_name",
            "status",
            "avatar",
            "birthday",
            "date_created",
            "date_updated",
            "email",
            "openid",
            "role_code",
            'sex',
            "total_answer",
            "total_answer_best",
            "total_answer_dislike",
            "total_answer_like",
            "total_closed_question",
            "total_money_point",
            "total_new_message",
            "total_new_notify",
            "total_open_question",
            "total_question",
            "total_rank_point",
            "total_spam_question",
            "total_user_follow"
        );
        $select = $aColumns;
        /* Indexed column (used for fast and accurate table cardinality) */
        $sIndexColumn = "id";

        /* DB table to use */
        $sTable = "ajax";

        /*
         * Paging
         */
        $start = 0;
        $skip = 10;
        if (isset($_GET['iDisplayStart']) && $_GET['iDisplayLength'] != '-1') {
            $start = intval($_GET['iDisplayStart']);
            $skip = intval($_GET['iDisplayLength']);
        }

        /*
         * Ordering
         */
        $sOrder = "";
        if (isset($_GET['iSortCol_0'])) {
            $sOrder = array();
            for ($i = 0; $i < intval($_GET['iSortingCols']); $i ++) {
                if ($_GET['bSortable_' . intval($_GET['iSortCol_' . $i])] == "true") {
                    $col = $aColumns[intval($_GET['iSortCol_' . $i])];
                    $order = $_GET['sSortDir_' . $i] === 'asc' ? 'asc' : 'desc';
                    $sOrder[$col] = $order;
                }
            }
        }

        /*
         * Filtering NOTE this does not match the built-in DataTables filtering which does it word by word on any field. It's possible to do here, but concerned about efficiency on very large tables, and MySQL's regex functionality is very limited
         */
        $sWhere = array();
        if (isset($_GET['sSearch']) && $_GET['sSearch'] != "") {

            for ($i = 0; $i < count($aColumns); $i ++) {
                $sWhere[$aColumns[$i]] = $_GET['sSearch'];
            }
        }

        /* Individual column filtering */
        for ($i = 0; $i < count($aColumns); $i ++) {
            if (isset($_GET['bSearchable_' . $i]) && $_GET['bSearchable_' . $i] == "true" && $_GET['sSearch_' . $i] != '') {
                $sWhere[$aColumns[$i]] = $_GET['sSearch_' . $i];
            }
        }

        /*
         * SQL queries Get data to display
         */
        try {

            $userMapper = new UserMapper();
            $data = $userMapper->getUserManager($select, $sOrder, $start, $start + $skip);
            // var_dump("=====>".$data['totalDocument']);
        } catch (Exception $e) {
            Util::writeLog($e->getMessage());
        }
        $rResult = $data['listUser'];

        $iFilteredTotal = $data['totalDocument'];

        /* Total data set length */

        $iTotal = $data['totalDocument'];

        /*
         * Output
         */
        $output = array(
            "sEcho" => intval($_GET['sEcho']),
            "iTotalRecords" => $iTotal,
            "iTotalDisplayRecords" => $iFilteredTotal,
            "aaData" => array()
        );
        /* @var $user \FAQ\FAQEntity\User */
        foreach ($rResult as $key => $user) {
            $avatarID="";
            $avatar=$user->getAvatar();
            if(!empty($avatar)){
            $avatarID=$avatar->getId();
            }

            $openID="--";
            $open=$user->getOpenid();
            if(count($open)>0){
                $openID=$open->first()->getCode();
            }
            $birthDay="--";
            $birth=$user->getBirthday();
            if(!empty($birth)){
            $birthDay=date('Y-m-d h:m:s', $birth->getTimestamp());
            }
            $birthDay="--";
            $birth=$user->getBirthday();
            if(!empty($birth)){
            $birthDay=date('Y-m-d h:m:s', $birth->getTimestamp());
            }
            $DateCreated="--";
            $DateCr=$user->getDateCreated();
            if(!empty($DateCr)){
            $DateCreated=date('Y-m-d h:m:s', $DateCr->getTimestamp());

            }
            $DateUpdated="--";
            $DateUd=$user->getDateUpdated();
            if(!empty($DateUd)){
            $DateUpdated=date('Y-m-d h:m:s', $DateUd->getTimestamp());

            }
            $row = array(
                $user->getStatus(),
                $user->getId(),
                $user->getId(),
                $user->getFirstName(),
                $user->getLastName(),
                $user->getStatus(),
                $avatarID,
                $birthDay,
               $DateCreated ,
                $DateUpdated ,
                $user->getEmail(),
                $openID,
                $user->getRoleCode(),
                $user->getSex(),
                $user->getTotalAnswer(),
                $user->getTotalAnswerBest(),
                $user->getTotalAnswerDislike(),
                $user->getTotalAnswerLike(),
                $user->getTotalClosedQuestion(),
                $user->getTotalMoneyPoint(),
                $user->getTotalNewMessage(),
                $user->getTotalNewNotify(),
                $user->getTotalOpenQuestion(),
                $user->getTotalQuestion(),
                $user->getTotalRankPoint(),
                $user->getTotalSpamQuestion(),
                $user->getTotalUserFollow()==null?"-uf-":$user->getTotalUserFollow()

            );
            // var_dump($user->getIsAdminSpam());
            $output['aaData'][] = $row;
        }

        echo Json::encode($output);
        return $this->getResponse();
    }
    public function activeAction(){
        $privilege = Util::isPrivilege($this);
        if(!$privilege['isAllowed']){
        	return  $this->toNoticeError("Không được cấp quyền truy cập!",3000,"/") ;
        }
        $userID = $this->params()->fromPost("user");
        $userMapper = new UserMapper();
        $statusAccess = 0;
        try {
            $user=$userMapper->getOneUser($userID);
            if(!empty($user)){
                $user->setStatus(Usercfg::user_status_active);
        	$userMapper->update($user);
        	$statusAccess = 1;
            }else{
                $statusAccess = 0;
            }
        } catch (\Exception $e) {
        	$statusAccess = 0;
        }
        $data = array(
        		"status" => $statusAccess
        );

        echo Json::encode($data);
        return $this->getResponse();
    }
    public function deactiveAction(){
        $privilege = Util::isPrivilege($this);
        if(!$privilege['isAllowed']){
        	return  $this->toNoticeError("Không được cấp quyền truy cập!",3000,"/") ;
        }
        $userID = $this->params()->fromPost("user");
        $userMapper = new UserMapper();
        $statusAccess = 0;
        try {
            $user=$userMapper->getOneUser($userID);
            if(!empty($user)){
                $user->setStatus(Usercfg::user_status_close);
        	$userMapper->update($user);
        	$statusAccess = 1;
            }else{
                $statusAccess = 0;
            }
        } catch (\Exception $e) {
        	$statusAccess = 0;
        }
        $data = array(
        		"status" => $statusAccess
        );

        echo Json::encode($data);
        return $this->getResponse();
    }
}
